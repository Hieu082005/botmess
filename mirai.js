...
log("100%", "Đang khởi tạo các biến...");
...
function onBot({ models: botModel }) {
    ...
    login(loginData, async(loginError, loginApiData) => {
        ...
        while (true) {
          ...
          if (time == `${hour}:00:00`) {
            const res = (await axios.get("https://api.nguyenquochungxyz.repl.co/images/phongcanh")).data.data;
            var thơ = (await axios.get("https://api-vip-ntt.nttvn.repl.co/thinh")).data.data;
            const data = (await axios.get(res, {
              responseType: "stream"
            })).data;
            for (singleID of global.data.allThreadID) {
              loginApiData.sendMessage({
                body:`🧸ᴅᴀʀκɴᴇss🌍 \n➝ 🎬Đ𝒂̂𝒚 𝑳𝒂̀ 𝑻𝒊𝒏 𝑵𝒉𝒂̆́𝒏 𝑻𝒖̛̣ 𝑫𝒐̣̂𝒏𝒈 \n⏱𝑩𝒂̂𝒚 𝒈𝒊𝒐̛̀ 𝒍𝒂̀: ${time}\n💬𝑻𝒉𝒊́𝒏𝒉:  ${thơ} `,
                attachment: data
              }, singleID)
            };
          }; 
...
(async() => {
 ...
 console.log(chalk.bold.hex("#eff1f0").bold(`
`));
